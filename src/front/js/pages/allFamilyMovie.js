...
export const GetFamilyMovies = () => {
  ...
  useEffect(() => {
    const fetchFamilyMovies = async () => {
      try {
        ...
        console.error("Error al obtener los datos de las películas familiares:", error);
      }
    };
  ...
  if (!familyMovies || familyMovies.length === 0) {
    console.log("No hay datos de películas familiares disponibles");
    return <div>No hay películas familiares disponibles</div>;
  }

  const indexOfLastMovie = currentPage * moviesPerPage;
  const indexOfFirstMovie = indexOfLastMovie - moviesPerPage;
  const currentMovies = familyMovies.slice(indexOfFirstMovie, indexOfLastMovie);
  ...
  return (
    <div className="container-fluid text-left mt-45" style={{ width: "90vw", marginTop: "7rem" }}>
      <div className="tittleRow d-flex justify-content-between mb-3">
        <h2><strong>Family Movies</strong></h2>
      ...
      <nav>
        <ul className="pagination justify-content-center mt-4">
          ...
          {Array.from({ length: Math.ceil(familyMovies.length / moviesPerPage) }, (_, i) => (
            <li className={`page-item ${currentPage === i + 1 ? "active" : ""}`} key={i}>
              <button className="page-link" onClick={() => paginate(i + 1)}>{i + 1}</button>
            </li>
          ))}
          <li className={`page-item ${currentPage === Math.ceil(familyMovies.length / moviesPerPage) ? "disabled" : ""}`}>
